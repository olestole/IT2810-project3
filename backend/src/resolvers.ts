...
export const resolvers = {
  Query: {
    ...
    filterProducts: async (_ :any, { varetyper, prisgt, prisls, volumgt, volumls }: any) => await Product.find({Varetype: {"$in": varetyper }, Pris: {"$gte": prisgt, "$lte": prisls}, Volum: {"$gte": volumgt, "$lte": volumls}}).limit(200),
  },
